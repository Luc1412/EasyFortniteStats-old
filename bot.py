...
import discord
from discord import Color, Embed, ActivityType, Forbidden, TextChannel
...
class LangManager:
    ...
    def __init__(self):
        with open('DE_de.json') as f:
            self.german_messages = json.load(f, encoding='utf-8')
        with open('EN_us.json') as f:
            self.english_messages = json.load(f, encoding='utf-8')
...
class FnStatsCommand:
    ...
    async def ex(self, user, channel, args):
        ...
        if len(args) is 2:
            ...
            # "Gespielte Spiele" Idee bei LACE
    ...
    async def ex(self, member, channel, args):
        ...
        embed = Embed(color=Color.dark_gold(),
                      description=messages['Desc'].format('EasyFortniteStats',
                                                          ...
                                                          1.0,
                                                          len(bot.guilds),
                                                          stats_manager.get_guild_amount(),
...
# /fnstats <platform> <Name> -> Stats Global
# /fnstats <platform> <Name> <Modus(Solo, Duo, Squad)> -> Stats Global nach Modus
# /fnstats <platform> <Name> <Modus(Solo, Duo, Squad)> <Art(all,4,3) -> Stats Global mit art
...
async def on_ready():
    activity = discord.Activity(type=ActivityType.listening,
                                name='{}fnhelp'.format(config_manager.get_config_value('General.Prefix')))
    await bot.change_presence(activity=activity)
    if bool(config_manager.get_config_value('CollectStats')):
        sched = PeriodicScheduler()
        sched.setup(15000, stats_manager.set_guild_amount)
